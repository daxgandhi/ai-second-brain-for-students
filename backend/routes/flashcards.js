...
const { generateFlashcardsWithPython } = require('../utils/pythonAiClient');
...
router.post('/', protect, async (req, res) => {
    try {
        ...
        const count = Math.min(parseInt(numCards) || 5, 20);
        const topicName = sourceTopic || 'Study Topic';
        let flashcards = [];

        try {
            // 🐍 Try Python AI Microservice first
            console.log('Sending flashcards request to Python AI Microservice...');
            const pyRes = await generateFlashcardsWithPython(sourceText || topicName, count);
            flashcards = pyRes.cards;
            console.log('✅ Flashcards generated by Python FastAPI AI Microservice');
        } catch (pyErr) {
            console.warn('Python AI service flashcard call failed, falling back to direct Gemini SDK:', pyErr.message);
            ...
            const prompt = `Create exactly ${count} educational flashcards based on:\n${sourceText || topicName}\nReturn ONLY a valid JSON array of {"front": string, "back": string}`;
