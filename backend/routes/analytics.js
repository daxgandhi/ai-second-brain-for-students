...
router.get('/recommendation', protect, async (req, res) => {
    try {
        ...
        const weakTopicNames = weakTopicsAgg.map(t => t._id);

        try {
            // 🐍 Try Python AI Microservice first
            console.log('Sending recommendation request to Python AI Microservice...');
            const pyRes = await generateRecommendationsWithPython(weakTopicsAgg, weakTopicNames);
            const dominantTopic = weakTopicNames[0] || 'General Topics';
            const formattedRec = `You are weak in ${dominantTopic}\n→ Revise notes on ${dominantTopic}\n→ Practice flashcards\n→ Re-take topic quizzes`;
            console.log('✅ Study recommendations generated by Python FastAPI AI Microservice');
            return res.json({ success: true, recommendation: formattedRec });
        } catch (pyErr) {
            console.warn('Python AI service recommendation call failed, falling back to direct Gemini SDK:', pyErr.message);

            if (!process.env.GEMINI_API_KEY || process.env.GEMINI_API_KEY === 'your_gemini_api_key_here') {
                return res.json({ success: true, recommendation: `You are weak in ${weakTopicNames[0]}\n→ Revise notes\n→ Take 3 quizzes\n→ Practice flashcards` });
            }

            const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
            const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
            const prompt = `You are an AI Study Recommendation Engine. The user is currently weakest in: ${weakTopicNames.join(', ')}. Provide a short 4-line recommendation starting with "You are weak in [topic]" followed by 3 bullet points starting with →.`;

            const result = await model.generateContent(prompt);
            let recommendation = result.response.text().trim().replace(/\*\*/g, '');
